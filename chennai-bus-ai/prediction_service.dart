...
class PredictionService {
  ...
  Future<void> init() async {
    if (_loaded) return;
    try {
      final raw = await rootBundle.loadString('assets/data/model_rules.json');
      _rules = json.decode(raw) as Map<String, dynamic>;
      _loaded = true;
      print('[PredictionService] Loaded ${_rules!.length} routes');
    } catch (e) {
      print('[PredictionService] Failed to load model_rules.json: $e');
    }
  }
  ...
  CrowdingPrediction? predict({
    ...
    try {
      ...
      print('[PredictionService] Error predicting for $routeKey: $e');
